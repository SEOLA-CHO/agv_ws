...
class WheelSimulatorNode(Node):
    ...
    def _read_response_alpha(self) -> float:
        ...
        value = float(self.get_parameter("response_alpha").value)

        if not math.isfinite(value) or not 0.0 < value <= 1.0:
            self.get_logger().error(
                "Parameter 'response_alpha' must satisfy "
                "0 < response_alpha <= 1. Using default value 0.1."
            )
            return 0.1

        return value

    def wheel_command_callback(self, message: WheelCommands) -> None:
        """Process a fixed-order /wheel_commands message."""

        try:
            self.model.set_command(message.velocity_rad_s)
        ...
        if self.timeout_active:
            self.get_logger().info(
                "Valid wheel command received. Timeout state cleared."
            )
    ...
    def timer_callback(self) -> None:
        ...
        message.velocity_rad_s = simulated_velocity
        message.erpm = [0] * WHEEL_COUNT
        message.tachometer_counts = [0] * WHEEL_COUNT
        message.online = [True] * WHEEL_COUNT
        self.publisher.publish(message)

    def _handle_timeout(self, current_time) -> None:
        """Stop the command if no new command arrives in time."""

        if not self.has_received_command or self.timeout_active:
            return
